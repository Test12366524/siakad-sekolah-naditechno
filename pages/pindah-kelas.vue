...
const handleInsertBulk = async () => {
  ...
  const url = "siswa/bulk-pindah-kelas";
...
//   useApi(`level/pindah-kelas/${user.role_id}`).then(({ data }) => {
//     if(data == 0){
...
<template>
  <SaveBulkDialog
    ...
    title="Form Pindah Kelas"
    edit-title="Form Pindah Kelas"
    :default-form="form"
  ...
  >
    <VCol cols="12" md="6">
      <VAutocomplete
        ...
        :items="[{ text: 'Data kelas kosong', id: 'ppdb' }, ...kelasList]"
        item-title="text"
        item-value="id"
        required
        :clearable="!isEditing"
